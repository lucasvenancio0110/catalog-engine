...
const migration0019 = fs.readFileSync(
...
function openMaintenanceDiscoveryDatabase() {
  ...
  database.exec(migration0019);
  return database;
}

function insertReadyMaintenanceTenant(database, tenantId, createdAt, prepared = true) {
  database
...
describe('tenant data-plane fleet migration activation', () => {
  ...
  it('targets schema v7 and only discovers maintenance work for ready idle tenants', () => {
    ...
    expect(migrationRunnerSource).not.toContain('uploadTenantCatalogWorker');
  ...
  it('does not let older failed jobs monopolize bounded maintenance discovery', () => {
    ...
    database
      .prepare(
        `INSERT INTO tenant_data_plane_migration_jobs
         ...
         VALUES ('dpmig_failed',?1,7,'maintenance','failed',1,
    ...
    const candidates = database.prepare(MAINTENANCE_MIGRATION_DISCOVERY_SQL).all(7, 3, 2);

    expect(candidates).toEqual([{ tenant_id: eligibleTenant }]);
  });

  it('does not discover or retry maintenance until trusted CI promotes the command marker', () => {
    const database = openMaintenanceDiscoveryDatabase();
    const preparedTenant = 't_11111111111111111111';
    const unpreparedTenant = 't_22222222222222222222';
    insertReadyMaintenanceTenant(database, preparedTenant, '2000-01-01T00:00:00Z', true);
    insertReadyMaintenanceTenant(database, unpreparedTenant, '2001-01-01T00:00:00Z', false);
    database
      .prepare(
        `INSERT INTO tenant_data_plane_migration_jobs
          (job_id,tenant_id,target_schema_version,migration_kind,status,attempt_count,
           next_attempt_at,created_at,updated_at)
         VALUES ('dpmig_unprepared',?1,7,'maintenance','failed',1,NULL,
                 CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`
      )
      .run(unpreparedTenant);

    expect(database.prepare(MAINTENANCE_MIGRATION_DISCOVERY_SQL).all(7, 3, 5)).toEqual([
      { tenant_id: preparedTenant }
    ]);
    expect(database.prepare(DATA_PLANE_MIGRATION_DUE_SQL).all(6, 7, 3, 5)).toEqual([]);
  });
  ...
  it('gives newly pending maintenance work capacity ahead of older failed retries', () => {
    ...
    const insertJob = database.prepare(
      `INSERT INTO tenant_data_plane_migration_jobs
        (job_id,tenant_id,target_schema_version,migration_kind,status,attempt_count,
         next_attempt_at,created_at,updated_at)
       VALUES (?1,?2,7,'maintenance',?3,1,NULL,?4,?4)`
    );
    ...
    const due = database.prepare(DATA_PLANE_MIGRATION_DUE_SQL).all(6, 7, 3, 2);
  ...
  it('finishes maintenance on v7 without resuming historical onboarding or changing serving status', async () => {
    ...
    const get = vi.fn(() => ({
      async fetch(request) {
        if (new URL(request.url).pathname.endsWith('/schema-migrate')) {
          ...
          return Response.json({ ok: true, version: 3, schemaVersion: 7, applied: true });
        }
        queryCall += 1;
        const results =
          queryCall === 1
    ...
    expect(successSql).not.toContain('SET worker_version=?2');
  ...
  it('retries binding reads and keeps DDL on the tenant binding path', async () => {
    ...
    expect(result).toMatchObject({ outcome: 'success', schemaVersion: 7 });
  ...
  it('reconciles control state after D1 already completed v7 without replaying schema DDL', async () => {
    const { db, batches } = fakeControlDb(maintenanceContext());
    let d1Call = 0;
    const get = vi.fn(() => ({
      async fetch(request) {
        d1Call += 1;
        expect(new URL(request.url).pathname).not.toContain('schema-migrate');
        const queries = (await request.json()).batch;
  ...
  it('persists a bounded inspect-phase code when tenant dispatch is unreachable', async () => {
    const { db, batches } = fakeControlDb(maintenanceContext());
    const get = vi.fn(() => ({
      async fetch() {
    ...
    expect(result).toMatchObject({
      outcome: 'failed',
      error: 'tenant_d1_migration_inspect_dispatch_failed'
    });
    expect(batches).toHaveLength(1);
  });

  it('never falls back to administrative D1 REST for maintenance without dispatch', async () => {
    const { db, batches } = fakeControlDb(maintenanceContext());
    ...
    expect(result).toMatchObject({
      outcome: 'failed',
      error: 'tenant_d1_migration_inspect_dispatch_unbound'
    });
  ...
  it('persists a bounded verify-phase code after the v7 migration was accepted', async () => {
    const { db, batches } = fakeControlDb(maintenanceContext());
    let queryCalls = 0;
    const get = vi.fn(() => ({
      async fetch(request) {
