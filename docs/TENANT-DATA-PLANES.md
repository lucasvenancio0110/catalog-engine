...
Transport aborts are persisted separately from unreachable transport. Tenant schema maintenance phase-qualifies bounded `inspect`, runtime `prepare`, binding `apply` and `verify` codes so retained evidence identifies whether the failure occurred before DDL, while refreshing the internal capability, during the idempotent version batch or during final verification. Retryable timeout, unreachable and dispatch-transport failures receive at most three in-run attempts with exponential backoff and jitter; the durable job retry remains the outer recovery boundary. The binding migration command may also be retried because its version batches are transactional and replay-safe. Non-transport/schema-state errors are not retried. None of these codes exposes provider response text or credentials.
...
Production activation of a new fleet schema target requires a dedicated trusted-main maintenance canary, not only a fresh-tenant provisioning canary. The fleet canary starts from isolated ready v4 fixtures whose initial User Worker deliberately lacks the new schema-migration command, then lets the deployed cron discover the work without inserting migration jobs or Queue messages. It proves the automatic capability refresh, successful binding-native v4→v5 upgrade, controlled failure with LKG preservation, active-import exclusion, unchanged historical onboarding, exact schema ledger/staging tables, merchant override preservation and unrelated-tenant isolation. Unexpected failure retains opaque fixture evidence plus the bounded migration job code/attempt count for diagnosis; cleanup happens only after the complete proof passes. A trusted-main change to the canary workflow reruns the same production proof without redeploying unchanged application code, while runtime changes still require the normal successful-deploy trigger.
...
The trusted-main application pipeline uploads the account ID and API token as Worker secrets in the same Wrangler deployment as the exact tested code SHA, then verifies only the two `secret_text` binding names through the read-only Worker Script Settings API. The secret material exists for physical provider operations, fresh provisioning migrations and Workers for Platforms script preparation. Because User Worker upload is a Workers for Platforms API operation rather than a dispatch-binding management method, the platform Worker enables Cloudflare's `global_fetch_strictly_public` compatibility flag and the deploy gate verifies it. This exception is limited to administrative provider operations: maintenance D1 inspection/application/verification, import, classification and verification use `TENANT_DISPATCH` and must not be changed back to administrative D1 REST access.
