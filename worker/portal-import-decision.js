...
async function enqueueInstantSeed(env, tenantId) {
  ...
  // Existing unit fixtures and old rollback deployments do not bind the IC2 queue.
  // Production deployment verifies the binding before this fast path is relied on.
  ...
  try {
    ...
    // The import decision is already durable. Returning a retryable error means a
    // repeated PUT safely re-enqueues the same minimal idempotent seed message.
