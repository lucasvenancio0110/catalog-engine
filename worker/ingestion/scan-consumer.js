...
async function fanOutDetails(db, context, platform, queue, fetchImpl) {
...
export async function handleTenantImportScanMessage(
  ...
  if (!env.TENANT_IMPORT_QUEUE || typeof env.TENANT_IMPORT_QUEUE.sendBatch !== 'function') {
    return { outcome: 'failed', error: 'tenant_import_queue_unbound' };
  }
  ...
  try {
    ...
    // Successful fan-out already clears the lease. Failure path clears it as well.
    // This fallback protects malformed/early-return cases where a lease was acquired.
    if (env.CATALOG_DB) {
      await releaseScanLease(env.CATALOG_DB, message.importId, message.tenantId).catch(() => {});
    }
  }
}
