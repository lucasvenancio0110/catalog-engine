...
const TENANT_ID = String(
  process.env.RETAINED_M7D3_TENANT_ID || 't_294b6cefe10ef209be1b'
).trim();
...
if (!/^t_[a-f0-9]{20}$/.test(TENANT_ID)) throw new Error('m7d3_diagnostic_tenant_invalid');
...
const control = await controlBatch([
  {
    sql: `SELECT import_id, source_key, mode, status, phase, last_error_code, created_at, updated_at
  ...
  {
    sql: `SELECT d1_database_id, worker_status, database_status
...
const jobs = rows(control, 0);
const failedJob = jobs.find((row) => String(row.last_error_code || '') === 'sync_stage_count_mismatch') || jobs[0];
const provider = rows(control, 1)[0] || null;
...
if (!importId || !sourceKey) throw new Error('m7d3_diagnostic_job_identity_invalid');
...
const tenant = await tenantBatch(databaseId, [
  {
    sql: `SELECT state, safety_outcome, observed_count, staged_observation_count,
           ...
           LIMIT 1`,
    params: [importId, TENANT_ID, sourceKey]
  },
  {
    sql: `SELECT COUNT(*) AS total
            FROM supplier_sync_stage_observations
           WHERE run_id=?1`,
    params: [importId]
  },
  {
  ...
  JSON.stringify(
    {
      ...
      dataPlane: {
        workerStatus: String(provider?.worker_status || ''),
        databaseStatus: String(provider?.database_status || '')
      },
      stage: {
