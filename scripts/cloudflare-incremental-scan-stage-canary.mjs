...
if (!/^[a-f0-9]{32}$/i.test(ACCOUNT_ID)) throw new Error('m7d3_canary_account_unconfigured');
if (API_TOKEN.length < 20) throw new Error('m7d3_canary_token_unconfigured');
...
if (!/^[a-f0-9-]{32,40}$/i.test(CONTROL_DB_ID)) {
  throw new Error('m7d3_canary_control_database_invalid');
}
if (String(wrangler.vars?.TENANT_IMPORT_AUTOMATION_ENABLED || '') !== '1') {
  throw new Error('m7d3_canary_import_automation_must_remain_on');
}
if (String(wrangler.vars?.TENANT_SYNC_AUTOMATION_ENABLED || '') !== '0') {
  throw new Error('m7d3_canary_recurring_sync_must_remain_off');
}
if (String(wrangler.vars?.TENANT_SYNC_ACTIVE_COHORT || '') !== '') {
  throw new Error('m7d3_canary_active_cohort_must_remain_empty');
}
if (String(wrangler.vars?.TENANT_SYNC_MAX_JOBS_PER_TICK || '') !== '1') {
  throw new Error('m7d3_canary_sync_cap_must_remain_one');
}
...
async function cloudflareRequest(path, { method = 'GET', allowNotFound = false } = {}) {
  ...
  try {
    ...
    throw new Error('m7d3_canary_cloudflare_unreachable');
  }
  ...
  if (!response.ok || payload?.success !== true) {
    ...
    throw new Error(`m7d3_canary_cloudflare_${code}`);
  }
...
function fixtureIdentity() {
  ...
  const suffix = createHash('sha256').update(`m7d3-canary:${seed}`).digest('hex').slice(0, 20);
  return {
    tenantId: `t_${suffix}`,
    workerScriptName: `ce-${suffix}`,
    databaseName: `cem7d3-${suffix}`,
    dataPlaneKey: `m7d3-${suffix}`
  };
}
...
async function loadQueues() {
  ...
  for (const name of QUEUE_NAMES) if (!queues.has(name)) throw new Error('m7d3_canary_queue_missing');
  return queues;
}
...
async function waitQueuesClean(queues) {
  ...
  throw new Error('m7d3_canary_queue_did_not_drain');
}
...
async function discoverSmallScope() {
  ...
  try {
    ...
    throw new Error('m7d3_canary_private_source_unavailable');
  }
  if (root.protocol !== 'https:' || !/\.x\.yupoo\.com$/i.test(root.hostname)) {
    throw new Error('m7d3_canary_private_source_invalid');
  }
  ...
  throw new Error('m7d3_canary_small_source_scope_not_found');
}
  ...
  await controlBatch([
    {
      sql: `INSERT INTO catalog_tenants
            ...
            VALUES (?1, ?2, 'M7D3 Canary', 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
      params: [fixture.tenantId, `m7d3-${fixture.tenantId.slice(2)}`]
    },
    {
...
async function controlState(fixture) {
  const result = await controlBatch([
    {
      sql: `SELECT status, phase, attempt_count, discovered_count, last_error_code
...
async function waitForStage(fixture) {
  ...
  while (Date.now() - started < DISPATCH_TIMEOUT_MS) {
    ...
    if (!job) throw new Error('m7d3_canary_job_missing');
    if (job.status === 'failed') {
      const safe = String(job.last_error_code || 'm7d3_canary_job_failed');
      throw new Error(/^[a-z0-9_]+$/i.test(safe) ? safe : 'm7d3_canary_job_failed');
    }
    const stage = await stageState(fixture);
    if (job.status === 'details' && job.phase === 'details' && stage?.state === 'details_pending') {
      return { job, stage };
    }
    await sleep(POLL_MS);
  }
  throw new Error('m7d3_canary_dispatch_timeout');
}
...
async function cleanupFixture(fixture) {
  if (fixture.controlCreated) {
    await controlBatch([
      { sql: 'DELETE FROM tenant_import_jobs WHERE import_id=?1 AND tenant_id=?2', params: [fixture.importId, fixture.tenantId] },
      { sql: 'DELETE FROM tenant_data_plane_provider_state WHERE tenant_id=?1', params: [fixture.tenantId] },
      { sql: 'DELETE FROM supplier_sources WHERE tenant_id=?1 AND source_key=?2', params: [fixture.tenantId, SOURCE_KEY] },
...
if (!queuesClean(await queueBacklogs(queues))) throw new Error('m7d3_canary_queue_not_empty_at_start');
...
try {
  ...
  if (before.lkgHash !== after.lkgHash || before.lkgCount !== after.lkgCount) {
    throw new Error('m7d3_canary_canonical_lkg_changed');
  }
  if (before.catalogCount !== after.catalogCount) throw new Error('m7d3_canary_catalog_changed');
  if (Number(state.stage.expected_detail_count || 0) !== 1) {
    throw new Error('m7d3_canary_affected_detail_count_invalid');
  }
  if (state.stage.safety_outcome !== 'proceed') throw new Error('m7d3_canary_safety_not_proceed');
  const summary = {
  ...
  console.error(
    JSON.stringify({
      incrementalScanStageCanaryPassed: false,
