...
import { verifyCatalogSourceUrl } from './catalog-source-resolver.mjs';
...
const sourceProvider = String(process.env.SOURCE_PROVIDER || '').trim().toLowerCase() || null;
...
let plan = buildTenantSourceConnection({
  tenantId,
...
if (validateNetwork) {
  const resolvedUrl = await verifyCatalogSourceUrl(
    plan.connection.provider,
    plan.privateSource.canonicalUrl
  );
  plan = buildTenantSourceConnection({
    tenantId,
