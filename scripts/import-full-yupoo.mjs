...
function albumMeta(html, album) {
  ...
  return { name, description, images, classification: classifyCatalogItem({ name, description, sourceImageCount: images.length }) };
}
...
async function scanListing({ baseUrl, maxPages, albums, category = null, rootOrderCounter = null }) {
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    for (const album of pageAlbums) {
      ...
      if (maxProducts > 0 && albums.size >= maxProducts) return { pages, naturalEnd: false, limited: true };
    }
...
function sourceAncestors(sourceCategoryId, rawById) {
  const output = [];
  const seen = new Set();
...
const rawCategoryById = new Map(rawTaxonomy.map((category) => [String(category.id), category]));
...
if (!rootResult.naturalEnd && !rootResult.limited && rootResult.pages >= maxRootPages) {
  throw new Error(`Varredura geral atingiu MAX_ROOT_PAGES=${maxRootPages} antes do fim natural. Aumente o limite para garantir catálogo completo.`);
}
...
console.log(`Descoberta concluída: ${albumList.length} álbuns únicos. Lendo detalhes e mídia sem baixar imagens...`);
...
if (failedAlbums > 0) {
  throw new Error(`Importação completa abortada: ${failedAlbums} álbum(ns) falharam. Um catálogo completo não pode inferir a saída com falhas de extração.`);
}
...
try {
  ...
  // Keep the public store configuration already present in the catalog.
}
...
if (/x\.yupoo\.com|photo\.yupoo\.com/i.test(serialized)) {
  throw new Error('White-label gate: URL/hostname do fornecedor apareceu no catálogo público.');
}
...
const summary = {
  ...
  rootPagesScanned: rootResult.pages,
  sourceCategoryPagesSafetyLimit: maxCategoryPages
};
